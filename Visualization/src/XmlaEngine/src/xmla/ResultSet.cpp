...
ResultSet::ResultSet(std::string MDXString,Properties props)
{	
	...
	//set schema
	if(Catalog!="")
		XSchemas::instance().getSchema(Catalog);
 			
	auto_ptr<QueryComputation>  temp(new QueryComputation(MDXString,Catalog));
	...
	for(int i=0;i<cellset.Cells.size();i++){
		...
		cout<<"formatString:"<<cellset.Cells[i].formatString<<"\n";
		cout<<"getformatString:"<<cellset.Cells[i].getFormatString()<<"\n";
		cout<<"isNull:"<<cellset.Cells[i].isNull<<"\n";
		cout<<"value:"<<cellset.Cells[i].value<<"\n";
	}
...
void ResultSet::serializeCellData(SoapSerializer * SoapSer){
	...
	for(int i=0;i<cellset.Cells.size();i++){
		...
		SoapSer->serialize("<FmtValue>",cellset.Cells[i].getFormatString().c_str(),"</FmtValue>\r\n",NULL);
		//Cell end
...
//返回cell的value
string ResultSet::getValue(const QueryComputationMethods::Cell& cell){

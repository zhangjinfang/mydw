...
int deOlaptype(void * *mObject,const  void * paramNode){
...
int deAlgorithm(void * *mObject,const  void * paramNode){
...
int seOlaptype(void * rs, SoapSerializer *serializer, bool bArray){
...
int seAlgorithm(void * rs, SoapSerializer *serializer, bool bArray){
...
int PreComputeMethod::invoke(list<Param> params,void ** pResult,void ** pSZFunct,string &mURI,string &rURI){
	...
	it++;
	pParam=*it;
	string *olaptype=(string *)(pParam.m_Value.pCplxObj->pObject);
  
	it++;
	pParam=*it;
	string *algorithm=(string *)(pParam.m_Value.pCplxObj->pObject);
  	 
	cout<<"start preCompute!\n";
	
    string rets = p.preCompute(*schemaName, *cubeName);//,*olaptype,*algorithm);
...
void PreComputeMethod::setUserTypeSeAndDeMethod(){
	...
	setSeMethod("olaptype",seOlaptype);
	setDeMethod("olaptype",deOlaptype);

	setSeMethod("algorithm",seAlgorithm);
	setDeMethod("algorithm",deAlgorithm);

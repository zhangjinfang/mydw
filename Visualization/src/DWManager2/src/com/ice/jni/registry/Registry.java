...
Registry
	...
	listKeysCommand( RegistryKey topKey, String keyName )
		...
		try {
			Enumeration enum1 = subKey.keyElements();
			for ( int kIdx = 0 ; enum1.hasMoreElements() ; ++kIdx )
				{
				String keyStr = (String) enum1.nextElement();
				System.err.println
					( "Subkey[" + kIdx + "] = '" + keyStr + "'" );
				}
			}
	...
	listValuesCommand( RegistryKey topKey, String keyName )
		...
		try {
			Enumeration enum1 = subKey.valueElements();
			for ( int kIdx = 0 ; enum1.hasMoreElements() ; ++kIdx )
				{

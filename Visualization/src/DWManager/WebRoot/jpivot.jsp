...
<body bgcolor=white>
<form action="jpivot.jsp" method="post">

<%-- include query and title, so this jsp may be used with different queries --%>
<%
	boolean bl = (Boolean)session.getAttribute("controlTestpage");
	if(bl){
		session.setAttribute("controlTestpage",false);
		
	String schemaName = (String)session.getAttribute("schemaName3");
	String cubeName = (String)session.getAttribute("cubeName3");
	...
	String mdx=cube.getInitMDX();
	
	System.out.println("MDX is:\n"+mdx);
